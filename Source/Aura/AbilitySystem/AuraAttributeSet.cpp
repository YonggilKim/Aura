...
{
	//Source = Effect를 발동시킨 액터
	//Target = Effect를 적용하는 대상

	Props.EffectContexHandle = Data.EffectSpec.GetContext();
	Props.SourceASC = Props.EffectContexHandle.GetOriginalInstigatorAbilitySystemComponent();

	if (IsValid(Props.SourceASC) && Props.SourceASC->AbilityActorInfo.IsValid() && Props.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		Props.SourceAvartarActor = Props.SourceASC->AbilityActorInfo->AvatarActor.Get();
		Props.SourceController = Props.SourceASC->AbilityActorInfo->PlayerController.Get();
		if (Props.SourceController == nullptr && Props.SourceAvartarActor != nullptr)
		{
			if (const APawn* Pawn = Cast<APawn>(Props.SourceAvartarActor))
	...
	if (Data.Target.AbilityActorInfo.IsValid() && Data.Target.AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		Props.TargetChracter = Cast<ACharacter>(Props.TargetAvatarActor);
